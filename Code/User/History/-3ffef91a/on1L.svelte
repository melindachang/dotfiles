...
{#snippet entryItem(
  ...
  <div class="body__item">
    <div class="body__item-caption">
      <span class="body__item-caption-text minor-text">{key.toUpperCase()}: </span>
    </div>
...
{#snippet text(value: string)}
  <p class="body__item--text">{@html value}</p>
{/snippet}
...
<div class={['entry', isFailure && 'entry--failure']} bind:this={collapsible}>
  <div class="title" bind:clientHeight={titleHeight}>
    ...
    <button class="title_icon" onclick={() => (expanded = !expanded)}>
...
<style lang="scss">
  .entry {
    ...
    &--failure {
      .entry__date__icon {
        background: $graphic-color !important;
      }
      .entry__title__text {
        color: $graphic-color;
      }
